...
/**
 * Represents a single Instagram reel search result item.
 */
final class InstagramReelSearchItem
{
...
/**
 * Represents Instagram reels search response.
 */
final class ReelsSearch
{
    /**
     * @param list<InstagramReelSearchItem> $items
    ...
    public static function fromArray(array $data): self
    {
        ...
        foreach (($data['items'] ?? []) as $item) {
            if (is_array($item)) {
                $items[] = InstagramReelSearchItem::fromArray($item);
            }
        }
